...
var arrsr = [];
const predefinedWordsex2 = ['client', 'server'];
function addbtnt21() {
    var inpt21 = document.getElementById("inp21").value.toLowerCase();
    ...
    if (inpt21 == "") {
        ...
        if (isInserted) {
            ...
            if (predefinedWordsex2.includes(inpt21)) {
...
function addbtnt22() {
    ...
    /* if((mtval == 2) || (mtval == 3)){
        ...
        arrsr.push(objsval);
        arrobjex2 = [...new Set(arrsr)]; //removes duplicate
        arrmesg.push(inpt22);
       /*  console.log(arrsr);
        console.log(arrobjex2); */
        let newtr = document.createElement("tr");
        let newtdIDs = "objs" + objsval;
        let newtds = document.createElement("td");
        newtds.setAttribute("id", newtdIDs);
        newtr.setAttribute("data-value", objsval);
        newtds.setAttribute("data-value", objsval);
        ...
        newIconbtn.setAttribute("onclick", "removerowtrex2(this)");
        ...
        else if((objsval.toLowerCase()=="client") && (objrval.toLowerCase()=="server") && (mtval == "SYNC"))
        {
            document.getElementById('tbodyt23').appendChild(newtr);
        */
        if ((objsval != objrval) && (mtval == "SYNC")) {
            document.getElementById('tbodyt23').appendChild(newtr);
        }
        else if (objsval == objrval) {
            alert("Message passing is not possible between " + objsval + " and " + objrval);
        }
        else if (mtval != "SYNC") {
            alert("Message passing is not possible between " + objsval + " and " + objrval + ". Please check message type.");
        }
    }
...
function removerowtrex2(btndel) {
    if (typeof (btndel) == "object") {
       // var listItemValueex2 = $(btndel).closest("td").data("value");
       // console.log(listItemValueex2);
        $(btndel).closest("tr").remove();
        arrmesg.pop();
        //arrobjex2.pop();
        //console.log(arrobjex2.pop());
    }
...
function drawbtnex2() {
   ...
   // alert(arrobjex2);
    ...
    if ((lenobjs == 2) && (lenmsg == 3)) {
        ...
        alert("Enter Object name and message in table 3. \n\n Hint: From the given problem statement, there are two objects and three messages are passed between objects.");
    }
